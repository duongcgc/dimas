			...
			switch( $tab_type ) {
				case 'global':
					if( ! isset( $tabs[$tab->post_name] ) && $this->is_global_tab($tab_id) && ! empty($tab->post_content) ) {
						...
						$tabs[$tab->post_name]['callback'] = 'Dimas\Addons\Modules\Product_Tabs\FrontEnd::get_product_tab_content';
					}
					break;
				case 'product':
				...
				case 'custom':
					...
					if( ! isset( $tabs[$tab->post_name] ) && $this->is_custom_tab($tab_id, $has_content) ) {
						...
						$tabs[$tab->post_name]['priority'] = $index;
						$tabs[$tab->post_name]['tab_id'] = $tab_id;
						$tabs[$tab->post_name]['callback'] = 'Dimas\Addons\Modules\Product_Tabs\FrontEnd::get_product_custom_tab_content';
					}
					break;
				case 'default':
					if( isset( $tabs[$tab->post_name] ) ) {
