<?php
/**
 * Dimas_PageNav functions and definitions.
 *
...
class Dimas_PageNav {
	...
	public function get_breadcrumbs( $args = '' ) {
		$args = wp_parse_args(
			$args, array(
				'separator'         => \Dimas\Icon::get_svg( 'chevron-right', 'delimiter' ),
				'home_class'        => 'home',
				'before'            => '',
				'before_item'       => '',
				'after_item'        => '',
				...
				'labels'            => array(
					...
					'author'    => esc_html__( 'Author:', 'dimas' ),
					'day'       => esc_html__( 'Daily:', 'dimas' ),
					'month'     => esc_html__( 'Monthly:', 'dimas' ),
					'year'      => esc_html__( 'Yearly:', 'dimas' ),
				),
			)
		);

		$args = apply_filters( 'dimas_breadcrumbs_args', $args );
		...
		if ( ! $args['home_class'] ) {
			...
			$items[] = sprintf(
				...
				$args['before_item'],
				$args['home_class'],
				apply_filters( 'dimas_breadcrumbs_home_url', get_home_url() ),
				$args['labels']['home'],
				$args['after_item']
			);
		...
		elseif ( is_home() && ! is_front_page() ) {
			...
			if ( get_option( 'dimas_portfolio_page_id' ) ) {
				$title = get_the_title( get_option( 'dimas_portfolio_page_id' ) );
			} else {
				$title = esc_html__( 'Portfolio', 'dimas' );
			}
		...
		elseif ( is_day() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'dimas' ), $args['labels']['day'], get_the_date() )
			);
		} // Month archive
		elseif ( is_month() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'dimas' ), $args['labels']['month'], get_the_date( 'F Y' ) )
			);
		} // Year archive
		elseif ( is_year() ) {
			$items[] = sprintf(
				$item_text_tpl,
				sprintf( esc_html__( '%s %s', 'dimas' ), $args['labels']['year'], get_the_date( 'Y' ) )
			);
		} // Archive
		else {
			$items[] = sprintf(
