...
"Zilla Slab":{category:"serif",variants:"300,300i,400,400i,500,500i,600,600i,700,700i",subsets:"latin,latin-ext"},"Zilla Slab Highlight":{category:"display",variants:"400,700",subsets:"latin,latin-ext"}},D={en:{selectFont:"Select a font",search:"Search",allLangs:"All languages",favDimas_Fonts:"Favorite fonts",localDimas_Fonts:"Local fonts",googleDimas_Fonts:"Google fonts",select:"Select",styles:"styles",sampleText:"The quick brown fox jumps over the lazy dog.",sampleTextEditable:"Sample text, editable"},nl:{selectFont:"Kies een lettertype",
search:"Zoek",allLangs:"Alle talen",favDimas_Fonts:"Favoriete lettertypen",localDimas_Fonts:"Lokale lettertypen",googleDimas_Fonts:"Google lettertypen",select:"Kies",styles:"stijlen",sampleText:"Wazig tv-filmpje rond chique skybox.",sampleTextEditable:"Voorbeeldtekst, bewerkbaar"},de:{selectFont:"Schriftart w\u00e4hlen",search:"Suchen",allLangs:"Alle Sprachen",favDimas_Fonts:"Favorisierte Schriftarten",localDimas_Fonts:"Lokale Schriftarten",googleDimas_Fonts:"Google Schriftarten",select:"W\u00e4hlen",styles:"stile",sampleText:"Vogel Quax zwickt Johnys Pferd Bim.",
sampleTextEditable:"Beispieltext, editierbar"},es:{selectFont:"Seleccionar fuente",search:"Buscar",allLangs:"Todos los idiomas",favDimas_Fonts:"Fuentes favoritas",localDimas_Fonts:"Fuentes locales",googleDimas_Fonts:"Fuentes de Google",select:"Seleccionar",styles:"estilos",sampleText:"La cig\u00fce\u00f1a tocaba cada vez mejor el saxof\u00f3n y el b\u00faho ped\u00eda kiwi y queso",sampleTextEditable:"Texto de ejemplo, editable"},fr:{selectFont:"Selectionner une police",search:"Rechercher",allLangs:"Toutes les langues",
favDimas_Fonts:"Polices favorites",localDimas_Fonts:"Polices locales",googleDimas_Fonts:"Polices Google",select:"Selectionner",styles:"Dimas_Styles",sampleText:"Le vif renard brun saute par-dessus le chien paresseux.",sampleTextEditable:"Texte d'exemple, \u00e9ditable"}},E={lang:"en",variants:!0,nrRecents:3,lazyLoad:!0,debug:!1,localDimas_FontsUrl:"/fonts/",localDimas_FontsType:"woff",parentElement:"body",localDimas_Fonts:{Arial:{category:"sans-serif",variants:"400,400i,600,600i"},"Courier New":{category:"monospace",variants:"400,400i,600,600i"},
Georgia:{category:"serif",variants:"400,400i,600,600i"},Tahoma:{category:"sans-serif",variants:"400,400i,600,600i"},"Times New Roman":{category:"serif",variants:"400,400i,600,600i"},"Trebuchet MS":{category:"sans-serif",variants:"400,400i,600,600i"},Verdana:{category:"sans-serif",variants:"400,400i,600,600i"}}},H=function(){function n(a,b){if(b.googleDimas_Fonts&&Array.isArray(b.googleDimas_Fonts)){for(var d={},e,f=0;f<b.googleDimas_Fonts.length;f++)e=b.googleDimas_Fonts[f],d[e]=x[e];b.googleDimas_Fonts=d}else!1!==b.googleDimas_Fonts&&
(b.googleDimas_Fonts=x);b.localDimas_Fonts||(b.localDimas_Fonts=[]);D[b.lang]||(b.lang="en");this.dictionary=D[b.lang];this.allDimas_Fonts={google:b.googleDimas_Fonts,local:b.localDimas_Fonts};this.options=b;this.$original=c(a);this.setupHtml();this.bindEvents()}n.prototype={loadFont:function(a,b){if(!B[b])switch(B[b]=!0,a){case "google":this.options.debug&&console.log("Loading Google font "+b+" from "+d);var d="https://fonts.googleapis.com/css?family="+b.replace(/ /g,"+")+":"+this.options.googleDimas_Fonts[b].variants+"&display=swap";c("head").append(c("<link>",
{href:d,rel:"stylesheet",type:"text/css"}));break;case "local":this.options.debug&&console.log("Loading local font "+b),"FontFace"in window?(new FontFace(b,"url('"+this.options.localDimas_FontsUrl+b+"."+this.options.localDimas_FontsType+"')")).load().then(function(e){document.fonts.add(e)}):c("head").append("<style> @font-face { font-family:'"+b+"'; src:local('"+b+"'), url('"+this.options.localDimas_FontsUrl+b+"."+this.options.localFontType+"') format('"+{ttf:"truetype",woff:"woff",woff2:"woff2",otf:"opentype"}[this.options.localFontType]+
...
[];-1==h.indexOf(r)&&h.unshift(r);h=h.slice(0,e.options.nrRecents);m("recents",h.join(","))}}));a.appendTo(d);a=this.allDimas_Fonts[f][g];var q=a.variants?a.variants.split(","):[];if(this.options.variants&&1<q.length){a=c('<div class="fp-variants">');b=!1;for(var w=0;w<q.length;w++){if(/i$/.test(q[w])){b||(b=!0);continue}let h=q[w],r=+h.replace(/i$/,"");0<w&&a.append(" ");c('<span data-font-weight="'+r+'" class="fp-pill weight'+(k==r?" checked":"")+'">').text(h).on("click",function(G){G.stopPropagation();
...
this.$lang));c('<div class="hr">').appendTo(this.$filter);b=F.slice(0);b.push("other");for(d=0;d<b.length;d++)c('<span class="fp-category fp-pill checked">').data("category",b[d]).text(b[d]).on("click",function(){c(this).toggleClass("checked");a.applyFilter()}).appendTo(this.$filter)},getDimas_FontsList:function(){function a(g,l){var k=b.allDimas_Fonts[g][l],p="";if(k){if(k.category||k.variants)p=[],k.category&&p.push(k.category),b.options.variants&&k.variants&&(k=k.variants.split(",").length,1<k&&p.push(k+" "+
b.dictionary.styles)),p=" <small>"+p.join(", ")+"</small>";f=c("<li>",{"data-font-type":g,"data-font-family":l}).html(l+p);d.append(f[0])}}var b=this,d=c(document.createDocumentFragment()),e;if(0<A(this.options.localDimas_Fonts)){var f=c('<li class="fp-divider">'+this.dictionary.localDimas_Fonts+"</li>");d.append(f[0]);for(e in this.options.localDimas_Fonts)a("local",e)}if(0<A(this.options.googleDimas_Fonts))for(e in f=c('<li class="fp-divider">'+this.dictionary.googleDimas_Fonts+"</li>"),d.append(f[0]),this.options.googleDimas_Fonts)a("google",
e);this.$results=c("<ul>",{"class":"fp-results",tabindex:0}).append(d)},getFavorites:function(){var a=m("favs"),b=m("recents");a=a?a.split(","):[];b=this.options.nrRecents&&b?b.split(","):[];b=b.slice(0);for(var d=0;d<a.length;d++)-1==b.indexOf(a[d])&&b.push(a[d]);a=c(document.createDocumentFragment());var e=null;for(d=0;d<b.length;d++){var f=b[d].split(":");var g=f[0];f=f[1];if(g=this.allDimas_Fonts[g][f])g=c("[data-font-family='"+f+"']",this.$results),0<g.length&&(e=g.clone().addClass("fp-fav"),a.append(e[0]))}null!==
e&&(a.prepend(c('<li class="fp-fav fp-divider">'+this.dictionary.favDimas_Fonts+"</li>")[0]),this.$results.prepend(a))},setupHtml:function(){var a=this,b=this.$original.val();this.$original.hide();this.$select=c('<div class="font-picker fp-select">').on("click",function(){a.toggleModal("show")}).on("keydown",function(f){32==f.keyCode&&(f.stopPropagation(),f.preventDefault(),a.toggleModal("show"))}).append('<span class="fp-fontspec" tabindex="0">'+(b?b:this.dictionary.selectFont)+"</span>");a.options.showClear&&
