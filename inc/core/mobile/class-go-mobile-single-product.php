...
class Single_Product {
	...
	public function hooks() {
		...
		remove_action( 'woocommerce_single_product_summary', array( \Dimas\GO_Theme::instance()->get( 'woocommerce' )->get_template( 'single_product' ), 'open_summary_top_wrapper' ), 2 );
		remove_action( 'woocommerce_single_product_summary', array( \Dimas\GO_Theme::instance()->get( 'woocommerce' )->get_template( 'single_product' ), 'single_product_taxonomy' ), 2 );

		// Re-order the stars rating.
		remove_action( 'woocommerce_single_product_summary', 'woocommerce_template_single_rating', 3 );
		remove_action( 'woocommerce_single_product_summary', array( \Dimas\GO_Theme::instance()->get( 'woocommerce' )->get_template( 'single_product' ), 'close_summary_top_wrapper' ), 4 );
