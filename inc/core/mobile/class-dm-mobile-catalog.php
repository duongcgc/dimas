...
class Mobile_Catalog {
	...
	public function hooks() {
		...
		add_filter('dimas_get_catalog_layout', array( $this, 'get_catalog_layout' ));

		add_filter('dimas_get_catalog_toolbar_layout', array( $this, 'get_catalog_toolbar_layout' ));

		add_filter('dimas_get_catalog_localize_data', array( $this, 'get_catalog_localize_data' ));

		// Remove page header class
		add_filter( 'dimas_page_header_class', '__return_empty_string' );
		...
		add_action( 'woocommerce_before_shop_loop', array( \Dimas\DM_Theme::instance()->get( 'breadcrumbs' ), 'breadcrumbs' ) );
		...
		add_action( 'dimas_get_product_filters_modal_classes', array( $this, 'get_product_filters_modal_classes' ) );

		// remove banners
		remove_action( 'woocommerce_before_shop_loop', array(
			\Dimas\DM_Theme::instance()->get( 'woocommerce' )->get_template( 'catalog' ),
	 ...
	 * @return string
	 */
	public function get_catalog_localize_data($dimas_catalog_data) {
		$dimas_catalog_data['catalog_filters_sidebar_collapse_content'] = 1;

		return $dimas_catalog_data;
	}
	...
	public function page_header() {
		...
		if( in_array( 'breadcrumb', $items )) {
			\Dimas\DM_Theme::instance()->get( 'breadcrumbs' )->breadcrumbs();
		}
