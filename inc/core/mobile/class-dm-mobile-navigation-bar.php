...
class Navigation_Bar {
	...
	function __construct() {
		add_filter( 'dimas_site_footer_class', array( $this, 'footer_class' ) );
		...
		add_action( 'dimas_navigation_bar_before_atc_template', array(
			$this,
			'open_wrapper_product_type_variable'
		), 10 );
		add_action( 'dimas_navigation_bar_after_atc_template', array(
			$this,
			'close_wrapper_product_type_variable'
		), 30 );

		add_action( 'dimas_navigation_bar_content_atc_template', array( $this, 'get_content_navigation' ), 30 );
	}
		...
		foreach ( $items as $item ) {
			...
			switch ( $item ) {
				case 'cart':
					...
					\Dimas\DM_Theme::instance()->set_prop( 'modals', $item );
					break;
	...
	public function close_wrapper_product_type_variable() {
		...
		\Dimas\DM_Theme::instance()->get( 'woocommerce' )->get_template( 'product_loop' )->display_variation_dropdown();
	...
	public function get_content_navigation() {
		...
		if ( $product->is_type( 'variable' ) ) {
			echo sprintf(
				...
				\Dimas\Icon::get_svg( 'cart', 'cart', 'shop' ),
				esc_html__('Add to Cart', 'dimas')
			);
		} else {
			if ( $product->is_in_stock() ) {
