...
class GO_Page_Header {
	...
	public function __construct() {
		add_action( 'dimas_after_close_site_header', array( $this, 'show_page_header' ), 20 );
		add_action( 'get_the_archive_title', array( $this, 'the_archive_title' ), 30 );

		add_action( 'dimas_after_open_page_header', array( $this, 'show_image' ), 30 );

		add_action( 'dimas_page_header_content_item', array( $this, 'open_content' ), 10 );
		add_action( 'dimas_page_header_content_item', array( $this, 'show_content' ), 20 );
		add_action( 'dimas_page_header_content_item', array( $this, 'close_content' ), 30 );

		add_filter( 'dimas_page_header_class', array( $this, 'get_page_header_classes' ) );
		add_filter( 'dimas_single_product_toolbar_class', array( $this, 'get_single_product_toolbar_classes' ) );
	}
	...
	public function show_page_header() {
		if ( ! apply_filters( 'dimas_get_page_header', true ) ) {
			return;
		}
		if ( is_404() ) {
			return;
	...
	public function get_items() {
		...
		return apply_filters( 'dimas_get_page_header_elements', $this->items );
