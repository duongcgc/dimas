...
class DM_Menu {
	...
	public function dimas_get_social_link_svg( $uri, $size = 24 ) {
		return \DM_SVG_Icons::get_social_link_svg( $uri, $size );
	}
	...
	protected function get_header_items( $items, $options ) {
		...
		foreach ( $items as $item ) {
			...
			switch ( $item['item'] ) {
				case 'hamburger':
					\Dimas\DM_Theme::instance()->set_prop( 'modals', $item['item'] );
					break;
	...
	public function mobile_header_icons() {
		...
		foreach ( $icons as $icon ) {
			...
			switch ( $icon['item'] ) {
				case 'cart':
					\Dimas\DM_Theme::instance()->set_prop( 'modals', 'cart' );
				...
				case 'account':
					\Dimas\DM_Theme::instance()->set_prop( 'modals', 'account' );
				...
				case 'search':
					\Dimas\DM_Theme::instance()->set_prop( 'modals', 'search' );
